 ...
 * ```
 */
import { _tokenize, _setSSRMode } from './template.js';
import { isUnsafeUrl, isEventAttribute } from '../security/sanitize.js';
/* ────────────────── HTML escaping ────────────────── */
...
function tokensToString(tokens) {
    ...
    for (const token of tokens) {
        switch (token.kind) {
            case TK_OPEN: {
                if (tagOpen)
                    html += '>'; // close previous opening tag
                html += `<${token.tag}`;
                tagOpen = true;
                break;
            }
            ...
            case TK_ATTR: {
                ...
                if (name === 'style' && typeof value === 'object') {
                    const cssStr = Object.entries(value)
                ...
                if (name === 'class' && typeof value === 'object' && !Array.isArray(value)) {
                    const clsStr = Object.entries(value)
                        .filter(([, on]) => on)
                        .map(([n]) => n)
                        .join(' ');
            ...
            case TK_EXPR: {
                ...
                if (typeof value === 'object' && '_ssrHtml' in value) {
                    html += value._ssrHtml;
                    break;
                }
 ...
 * ```
 */
export function renderHTML(componentFn) {
    _setSSRMode(ssrHtml);
    try {
