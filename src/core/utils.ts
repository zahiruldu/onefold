...
export function formatDate(
  ...
  format: 'short' | 'long' | 'iso' | 'datetime' = 'short'
): string {
...
export function timeAgo(date: Date | number | string): string {
  ...
  else if (days === 1) label = isPast ? 'yesterday' : 'tomorrow';
  ...
  if (label === 'just now' || label === 'yesterday' || label === 'tomorrow') return label;
 ...
 * ```
 */
export function pipe<T>(value: T, ...fns: ((v: any) => any)[]): any {
