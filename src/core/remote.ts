...
export type RemotePermission =
  | 'dom'         // Can insert content into the host DOM (via shadow or direct)
  | 'storage'     // Can access localStorage/sessionStorage
  | 'network'     // Can make fetch/XHR requests
  | 'navigation'  // Can trigger navigate()
  | 'clipboard';  // Can access clipboard API
...
function validateOrigin(url: string): void {
  ...
  if (!trusted || trusted.length === 0) return; // No restriction configured
  ...
  if (!trusted.includes(origin)) {
    throw new Error(
      `[onefold:security] Blocked untrusted origin "${origin}". ` +
 ...
 */
async function fetchWithIntegrity(url: string, integrity?: string, timeoutMs?: number): Promise<string> {
  ...
  try {
    ...
    if (integrity) {
      ...
      if (!valid) {
        throw new Error(
          `[onefold:security] Integrity check FAILED for "${url}". ` +
 ...
 */
async function verifySRI(source: string, integrity: string): Promise<boolean> {
  ...
  const expectedHash = match[2] as string;
 ...
 */
async function loadModuleSecure(
  url: string,
  integrity?: string,
  timeoutMs?: number
  ...
  const promise = (async () => {
    if (integrity || securityConfig.requireIntegrity) {
      if (securityConfig.requireIntegrity && !integrity) {
        throw new Error(
          `[onefold:security] Integrity hash required for "${url}". ` +
      ...
      try {
        const mod = await import(/* webpackIgnore: true */ blobUrl) as Record<string, unknown>;
        return mod;
      } finally {
      ...
      return await (import(/* webpackIgnore: true */ url) as Promise<Record<string, unknown>>);
    }
  })();
...
function mountInIframe(
  ...
  const sandboxTokens: string[] = ['allow-scripts'];
  if (permissions.includes('navigation')) sandboxTokens.push('allow-top-navigation-by-user-activation');
 ...
 */
export function loadRemote<P = Record<string, unknown>>(
  ...
  return (props?: P): Node => {
    ...
    try {
      ...
      if (onError) {
        container.appendChild(onError(err as Error));
      } else {
        console.error(err);
    ...
    if (isolate === 'iframe') {
      ...
      mountInIframe(url, container, permissions, props as Record<string, unknown>);
      return container;
    }
    ...
    loadModuleSecure(url, integrity, timeout)
      .then((mod) => {
        ...
        const node = factory(props ?? {}) as Node;
        container.textContent = '';
 ...
 */
export function preloadRemote(url: string, integrity?: string): Promise<void> {
  try {
    validateOrigin(url);
  } catch (err) {
...
export function clearRemoteCache(url?: string): void {
  if (url) {
    for (const key of moduleCache.keys()) {
      if (key.startsWith(url)) moduleCache.delete(key);
    }
  } else {
    moduleCache.clear();
  }
