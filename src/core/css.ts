...
const cache = new Map<string, ScopedStyle>();
 ...
 */
function scopeCSS(raw: string, scopeClass: string): string {
  ...
  while (i < len) {
    // Skip whitespace
    while (i < len && /\s/.test(raw[i]!)) { result += raw[i]; i++; }
    if (i >= len) break;
 ...
 */
export function css(strings: TemplateStringsArray, ...values: unknown[]): ScopedStyle {
  ...
  const result: ScopedStyle = { scope: scopeClass, css: scopedCSS };
